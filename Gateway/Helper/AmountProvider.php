...
class AmountProvider
{
    ...
    {
        list($amount, $orderCurrency) = $this->checkCurrencyDifference( // @phpstan-ignore-line
            $amount,
            $orderCurrency,
            $paymentCurrency,
            $orderTotal,
    ...
    public function checkCurrencyDifference($amount, $orderCurrency, $paymentCurrency, $orderTotal, $paymentTotal)
    {
        ...
        $this->logger->debug('Currency difference check - initialAmount: ' . $amount .
                            ' - paymentCurrency: ' . $paymentCurrency .
                            ' - orderCurrency: ' . $orderCurrency);

        if ($paymentCurrency != $orderCurrency) {
            //if amount is total amount, use total payment amount as converted amount
            $isTotalAmount = $amount == $orderTotal;

            if ($isTotalAmount) {
                $this->logger->debug('use total payment amount as converted: - amount - ' . $amount .
                                    ' paymentTotal - ' . $paymentTotal . ' - paymentCurrency: ' . $paymentCurrency .
                                    ' - orderTotal: ' . $orderTotal . ' - orderCurrency: ' . $orderCurrency);
            ...
            $this->logger->debug('Currency difference check - convertedAmount: ' . $amount .
                                 ' - convertedCurrency: ' . $paymentCurrencyObj->getCode());
