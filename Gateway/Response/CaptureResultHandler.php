...
class CaptureResultHandler implements HandlerInterface
{
    ...
    public function handle(array $handlingSubject, array $response)
    {
        ...
        if (is_array($revolutOrder->revolutOrder) && in_array('errorId', array_keys($revolutOrder->revolutOrder))) {
            ...
            throw new LocalizedException(__('An error occured during capture action, errorId: '
                                            . $revolutOrder->revolutOrder['errorId']));
        }
        ...
        if ($payment instanceof Payment) {
            ...
            $this->logger->debug('CaptureResultHandler isPaymentCompleted: '
                                    . $isPaymentCompleted . ' '
                                    . $revolutOrder->revolutOrder['state']);

            $payment->setAdditionalInformation(
                ConstantValue::WEBHOOK_ACTION,
                $isPendingPayment ? ConstantValue::WEBHOOK_ACTION_ACCEPT_PAYMENT
                : ConstantValue::WEBHOOK_ACTION_PAYMENT_COMPLETED
            );
