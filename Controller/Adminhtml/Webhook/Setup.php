...
class Setup extends Action
{
    ...
    /**
     * Address ValidationWebhookSetup
     *
     * @param object $store
     *
     * @return array
    ...
    public function addressValidationWebhookSetup($store)
    {
        try {
            ...
            if ($this->config->getAddressValidationWebhookSetupUrl($locationId, $storeId) === $webhookUrl) {
                ...
                $this->logger->debug('Address Validation webhook url exist storeId: ' .
                $storeId . ' storeCode: ' . $storeCode . ' url: ' . $webhookUrl);
            ...
            if (empty($result['signing_key'])) {
                throw new LocalizedException(__('Can not setup address validation webhook.'));
            }
            ...
            $this->logger->debug('Address Validation webhook url registered storeId: ' .
            $storeId . ' storeCode: ' . $storeCode . ' url: ' . $webhookUrl);

            return ['result' => true, 'storeCode' => $storeCode];
        } catch (\Exception $e) {
            $this->logger->debug('Address Validation webhook setup error storeId: ' .
            $store->getId() . ' storeCode: ' . $store->getCode() . ' - ' . $e->getMessage());
    ...
    public function setupLocation($store)
    {
        ...
        if (! isset($location['id']) || empty($location['id'])) {
            throw new LocalizedException(__('Can not create location object.'));
        }
    ...
    public function webhookSetup($storeId, $storeCode)
    {
        try {
            ...
            if (in_array($webhookUrl, array_column($webHookList, 'url'))) {
                foreach ($webHookList as $webhook) {
                    if (isset($webhook['url'])
                        ...
                        if (!isset($updateResult['id'])) {
                            throw new LocalizedException(__("Can not update webhook events: " .
                ...
                $this->logger->debug('webhook url exist storeId: ' . $storeId .
                ' storeCode: ' . $storeCode . ' url: ' . $webhookUrl);
            ...
            if (!isset($result['id'])) {
                throw new LocalizedException(__("Can not setup webhook url: " . \json_encode($result)));
            }

            $this->logger->debug('webhook setup storeId: ' . $storeId . ' storeCode - ' .
            $storeCode . ' - url -' . $webhookUrl);
            $this->config->saveWebhookSetupResult(1, $storeId);
            return ['result' => true, 'storeCode' => $storeCode];
        } catch (\Exception $e) {
            $this->logger->debug('webhookSetup error storeId: ' .
            $storeId . ' storeCode: ' . $storeCode . ' - ' . $e->getMessage());
