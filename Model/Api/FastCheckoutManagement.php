...
class FastCheckoutManagement implements FastCheckoutManagementInterface
{
     ...
     * @param RevolutPaymentRequestConfig $paymentRequestConfigHelper
     * @param RevolutPayConfig $revolutPayConfigHelper
     * @param RevolutConfig $revolutConfigHelper
     * @param RevolutApi $revolutApi
     * @param Resolver $localeResolver
     * @param ChangeQuoteControl $changeQuoteControl
     * @param RevolutOrderModelCollection $revolutOrderCollection
     * @param RevolutPaymentInformationManagement $revolutPaymentInformationManagement
     * @param FastCheckoutResponseDataInterfaceFactory $fastCheckoutResponseData
     * @param TotalsDataInterfaceFactory $totalsData
     * @param ShippingOptionDataInterfaceFactory $shippingOptionData
     * @param FastCheckoutParametersResponseDataInterfaceFactory $fastCheckoutParametersResponseData
     * @param ValidateOrderRequestDataInterfaceFactory $validateOrderRequestData
     * @param AddressDataInterfaceFactory $addressData
     */
    ...
    {
        if (class_exists(\Laminas\Uri\Uri::class)) {
            $uri = new \Laminas\Uri\Uri();
        } elseif (class_exists(\Zend\Uri\Http::class)) {
            $uri = new \Zend\Uri\Uri(); // @phpstan-ignore-line
        }

        $uri->setQuery($productForm); // @phpstan-ignore-line
        $params = $uri->getQueryAsArray(); // @phpstan-ignore-line
    ...
    public function addressValidationWebhook(
        ...
        try {
            ...
            if ($quote->isVirtual()) {
                throw new LocalizedException(__("virtual orders are not supported"));
            }
    ...
    public function loadShippingOptions(
        ...
        try {
            ...
            $rates = $this->shipmentEstimation->estimateByExtendedAddress(
                $quote->getId(),
    ...
    public function validateOrder(
        ...
        try {
            ...
            if (!$address) {
                ...
                $order = $this->revolutOrderApi->retrieve(
                    $revolutOrder->getRevolutOrderId(),
                    $revolutOrder->getStoreId()
                );
            ...
            $this->logger->debug('An error occured during order validation: order token - ' .
    ...
    public function updateRevolutOrder($publicId, $amount, $currency, $quote, $isRevolutPay = false)
    {
        ...
        $orderData = $this->revolutOrderApi->update(
            $revolutOrderId,
    ...
    /**
     * Is RevolutPaymentRequestActiveLocation
     *
     * @param string $location
     * @return bool
     */
