...
class OrderManagement implements OrderManagementInterface
{
    ...
    public function create()
    {
        try {
            ...
            $this->logger->debug('create order: ' . $amount . ' ' .
                                 $currency . ' storeId: ' . $storeId .
                                  ' customerId: ' . $customerId);
            
            $revolutOrderModel = $this->revolutOrderCollection->getOrderByQuoteAndStore(
    ...
    public function webhook(String $orderId, String $event)
    {
        ...
        try {
            ...
            if (empty($this->revolutOrder->revolutOrder['state'])
                || $this->revolutOrder->revolutOrder['state'] == 'PENDING') {
    ...
    public function placeOrder(PaymentInterface $paymentMethod)
    {
        try {
            ...
            if (empty($this->revolutOrder->getIncrementOrderId())) {
                $orderProcessingResult = $this->revolutPaymentInformationManagement->placeOrder(
                    $publicId,
                    $paymentMethod,
                    $setAgreement
                );
            ...
            if (empty($this->revolutOrder->getIncrementOrderId())) {
                $errorMsg = !empty($orderProcessingResult['errorMsg']) ?
                $orderProcessingResult['errorMsg']: 'Something went wrong.';
            ...
            $inProgress = isset($orderProcessingResult) && isset($orderProcessingResult['processing']) ?
                        $orderProcessingResult['processing'] : false;
            return $this->orderManagementResponse->create()
                    ->setSuccess(false)
    ...
    public function cancel(String $publicId, String $cancelReason)
    {
        try {
            ...
            $this->logger->debug('An error occured during order cancellation : ' .
