...
class RevolutPaymentInformationManagement
{
    ...
    public function placeOrder($publicId, ?PaymentInterface $paymentMethod = null, $setAgreement = false)
    {
        try {
            if (empty($publicId)) {
                throw new LocalizedException(__("public id is missing"));
            }
            ...
            if (empty($revolutOrder->getQuoteId())) {
                throw new LocalizedException(__("Can not find quote for public id: " . $publicId));
            }
            ...
            if (empty($storeId)) {
                throw new LocalizedException(__("Can not find store for public id: " . $publicId));
            }

            if (empty($revolutOrder->revolutOrder['state']) || $revolutOrder->revolutOrder['state'] == 'PENDING') {
                throw new LocalizedException(__("Order has pending state"));
            }
    ...
    {
        try {
            $this->revolutWebhookRequestFactory->create()
            ->load($publicId, 'public_id')->delete(); // @phpstan-ignore-line
